#ifndef msd_radix_sort_cpp
#define msd_radix_sort_cpp
...
static void msd_radix_sort(int shift, RAI a0, RAI aN, G get_key, C cmp, B b);

template <int UNIT, int BITS, class RAI, class G, class C>
void msd_radix_sort(RAI a0, RAI aN, G get_key, C cmp)
{
        ...
        try { msd_radix_sort<UNIT>(BITS - UNIT, a0, aN, get_key, cmp, b); }
...
static void msd_radix_sort(int shift, RAI a0, RAI aN, G get_key, C cmp, B b)
{
        ...
        if (shift > 0) {
                ...
                for (int k = 0; k < MASK; k++)
                        msd_radix_sort<UNIT>(shift,
                                a0 + h[k], a0 + h[k + 1], get_key, cmp, b);
                msd_radix_sort<UNIT>(shift, a0 + h[MASK], aN, get_key, cmp, b);
        }
        delete[] h;
}

#endif // msd_radix_sort_cpp